...
public: 
	virtual double evaluate(const VectorXd &u_stack) const { 
		...
		// Distance of x from sun in vector and normalized form
		auto d_from_sun = (x.back()-x_sun).normalized();
		auto v_from_sun = x.back()-x_sun;

		// Cost on distance from sun
		// r_prime is radius
		O += 10*(d_from_sun * r_prime - v_from_sun).squaredNorm();

		// Cost on velocity tangential to circle
		O += pow(v_from_sun.dot(v.back()),2);

		// Cost on velocity being equal to orbital velocity
		// Since properties of sun are unknown / not in simulation, the gravitational constant K is a parameter that is tuned by trial and error
		double k = 3.8;
		O +=  pow((v.back().norm()-std::sqrt(k*r_prime/m)),2);
