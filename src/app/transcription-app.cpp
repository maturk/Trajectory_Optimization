...
public:
	...
	double log_c_reg = -3.5; double log_c_reg_min = -6.; double log_c_reg_max = 6.;
	...
	virtual double evaluate(const VectorXd &stack) const {
		...
		double O = 0.; 
		// TODO: O += ...
		// Objective is the same as before but with additional soft constraints on dynamics, initial position, initial velocity...
		// ... as well as regularizer on the norm of [u x v].T
		// I carefully selected the weights with trial and error... 
		
		// // Position	
		O += (x.back() - x_prime).squaredNorm(); 
		// Velocity
		O += (v.back().squaredNorm());
		// Soft constraint on initial position
		O += 100* (x0-x[0]).squaredNorm();
		// Soft constraint on inital velocity
		O += 100* (v0 - v[0]).squaredNorm();
		// Regularizer on vector of optimization variables [u x v].T
		// Traj u, x, v are Eigen::Vector2d so no norm or dot product operations... manual for loop I guess...
		for (int i = 0; i < u.size(); i++ ){
			O += pow(10., log_c_reg) * (u[i].squaredNorm() + x[i].squaredNorm() + v[i].squaredNorm());
		}
		// Soft constraint on dynamics
		for (int i = 0; i < u.size()-1; i++ ){
			// Position dynamics
			//xkp1 = xk + h * vk;
			O += 100*(x[i+1]- x[i]-h*v[i]).squaredNorm();
			// Velocity dynamics
			// vkp1 = vk + h * get_Fk(uk, xk) / m;
			O += 100*(v[i+1]-v[i]-(h/m)*u[i]).squaredNorm();
		}
