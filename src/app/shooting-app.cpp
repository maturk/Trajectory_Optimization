...
Vector2d get_Fk(const Vector2d &uk, const Vector2d &xk) {
	...
	// TODO: Fk += ...
	// Fk is just the input at time k. With a planet nearby, we need to add the force from the planet as well.
	Fk += uk;
	if (PLANET) {
		Vector2d r = (x_planet - xk);
		double r_mag = r.norm();
		Fk += m * r / (r_mag * r_mag + 1e-3);
	}
	return Fk;
} 
...
public:
	...
	virtual double evaluate(const VectorXd &u_stack) const { 
		...
		double O = 0.;
		// TODO: O += ...
		// O is the objective function. It's just the sum of the squared error from x' (x_prime) and the squared error from v' (v_prime = 0).

		// Position	
		O += 5 * (x.back() - x_prime).squaredNorm(); 
		// Velocity
		O += 5 * (v.back().squaredNorm());
		// Regularizer on norm of U
		O += pow(10., log_c_reg) * u_stack.squaredNorm();	
		//std::cout<<u_stack<<std::endl;	
